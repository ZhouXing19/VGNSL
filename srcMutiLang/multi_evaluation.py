...
import numpy
from multi_data import get_eval_loader
import time
import numpy as np
from multi_vocab import Vocabulary
import torch
from multi_model import VGNSL
...
def t2i(images, captions, npts=None, measure='cosine', return_ranks=False):
    ...
    ranks = numpy.zeros(10 * npts)
    top1 = numpy.zeros(10 * npts)
    for index in range(npts):
        ...
        for i in range(len(inds)):
            ...
            ranks[10 * index + i] = numpy.where(inds[i] == index)[0][0]
            top1[10 * index + i] = inds[i][0]
